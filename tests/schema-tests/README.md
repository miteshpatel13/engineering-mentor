...
Add a new fixture by dropping a `<name>.yaml`/`<name>.md` file plus a `<name>.expected.json` sidecar into the relevant directory (or a new subdirectory under `child-rules/dir-fixtures/` plus its sidecar directly under `child-rules/`); the runners pick up new fixtures automatically -- no fixture list to maintain by hand.

See `docs/Child Rules and Exceptions.md` for the contracts these fixtures validate against, and `docs/Project Profile Schema.md` for the `project.yaml` contract.
