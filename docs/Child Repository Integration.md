...
- **`exceptions.yaml`** — the record of every explicit, approved deviation from a Mentor rule. See Section 17.
- **`rules/`** — child-specific rules that are Mandatory or Configurable *for this project* (project-specific business/domain rules, project-specific conventions elevated to required status) — the child-side counterpart to Mentor's own `context/`. Format is deferred (Section 22).
...
- Reconcile this Contract's Section 8/18 conflict-priority model with the coarser conflict-priority statements already present in `context/core/Mentor Operating Model.md` and `skills/mentor-development/SKILL.md` (see the ambiguity called out in this task's summary).
